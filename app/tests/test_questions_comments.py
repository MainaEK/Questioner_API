...
class TestQuestioner(unittest.TestCase):
    ...
    def test_post_comment(self):
        ...
        response = self.client.post('/api/v2/questions/1/comments', data=json.dumps(self.comment), headers = self.headers)
        self.assertEqual(response.status_code, 201)

    def test_post_comment_not_registered(self):
        """ Test posting a comment."""
        self.comment = {"comment" : "Good question"}
        response = self.client.post('/api/v2/questions/1/comments', data=json.dumps(self.comment), content_type = 'application/json')
